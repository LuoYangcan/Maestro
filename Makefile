...
SPM_CACHE_DIR := $(HOME)/Library/Caches/Maestro-spm-cache/SourcePackages
VERSION ?=
BUILD ?=
XCODEBUILD_FLAGS ?=
...
_check-ghostty-hash:
	...
	if [ "$$current_sha" != "$$last_sha" ] || [ "$$artifacts_ok" -ne 1 ]; then \
		...
		if [ "$$current_sha" != "$$last_sha" ]; then \
			rm -rf ~/Library/Developer/Xcode/DerivedData/Maestro-*; \
...
build-app: ensure-ghostty embed-cli-debug # Build the macOS app (Debug)
	bash -o pipefail -c 'xcodebuild -project Maestro.xcodeproj -scheme Maestro -configuration Debug build -skipMacroValidation -clonedSourcePackagesDirPath $(SPM_CACHE_DIR) 2>&1 | mise exec -- xcsift -w --format toon'

sync-cli-version: # Sync app MARKETING_VERSION into MaestroCLIShared/MaestroVersion.swift
	@version="$$(/usr/bin/awk -F' = ' '/MARKETING_VERSION = [0-9.]*;/{gsub(/;/,"",$$2);print $$2; exit}' \
		"$(CURRENT_MAKEFILE_DIR)/Maestro.xcodeproj/project.pbxproj")"; \
	dst="$(CURRENT_MAKEFILE_DIR)/Maestro/CLIService/Shared/MaestroVersion.swift"; \
...
build-cli: sync-cli-version # Build Swift CLI binary (SPM)
	swift build --product maestro

build-cli-release: sync-cli-version # Build universal CLI binary in release mode
	swift build -c release --arch arm64 --arch x86_64 --product maestro

embed-cli-debug: build-cli # Build debug CLI and copy into Resources for dev builds
	@set -euo pipefail; \
	bin="$$(swift build --show-bin-path)/maestro"; \
	dst="$(CURRENT_MAKEFILE_DIR)/Resources/maestro-cli"; \
	mkdir -p "$$dst"; \
	cp "$$bin" "$$dst/maestro"; \
	chmod +x "$$dst/maestro"; \
	echo "embedded CLI binary at $$dst/maestro"

embed-cli: build-cli-release # Build release CLI and copy into Resources for distribution
	@set -euo pipefail; \
	bin="$$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/maestro"; \
	dst="$(CURRENT_MAKEFILE_DIR)/Resources/maestro-cli"; \
	mkdir -p "$$dst"; \
	cp "$$bin" "$$dst/maestro"; \
	chmod +x "$$dst/maestro"; \
...
archive: build-ghostty-xcframework embed-cli # Archive Release build for distribution
	bash -o pipefail -c 'xcodebuild -project Maestro.xcodeproj -scheme Maestro -configuration Release -archivePath build/Maestro.xcarchive archive CODE_SIGN_STYLE=Manual DEVELOPMENT_TEAM="$$APPLE_TEAM_ID" CODE_SIGN_IDENTITY="$$DEVELOPER_ID_IDENTITY_SHA" OTHER_CODE_SIGN_FLAGS="--timestamp" -skipMacroValidation -clonedSourcePackagesDirPath $(SPM_CACHE_DIR) $(XCODEBUILD_FLAGS) 2>&1 | mise exec -- xcsift -qw --format toon'

export-archive: # Export xarchive
	bash -o pipefail -c 'xcodebuild -exportArchive -archivePath build/Maestro.xcarchive -exportPath build/export -exportOptionsPlist build/ExportOptions.plist 2>&1 | mise exec -- xcsift -qw --format toon'

test: ensure-ghostty embed-cli-debug test-app

test-app: ensure-ghostty # Run app/unit tests via xcodebuild
	@set -euo pipefail; \
	result_bundle="$(CURRENT_MAKEFILE_DIR)/build/test-results/Maestro-tests.xcresult"; \
	mkdir -p "$$(dirname "$$result_bundle")"; \
	rm -rf "$$result_bundle"; \
	set +e; \
	xcodebuild test -project Maestro.xcodeproj -scheme Maestro -destination "platform=macOS" -resultBundlePath "$$result_bundle" CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY="" -skipMacroValidation -clonedSourcePackagesDirPath $(SPM_CACHE_DIR) 2>&1 | mise exec -- xcsift -w --format toon; \
...
test-cli-smoke: build-cli # Smoke test CLI executable
	...
	socket="/tmp/maestro-cli-smoke-$$RANDOM.sock"; \
	MAESTRO_CLI_SOCKET="$$socket" "$$bin" list --json >/tmp/maestro-cli-smoke.json || true; \
	jq -e '.error.code == "APP_NOT_RUNNING"' /tmp/maestro-cli-smoke.json >/dev/null

test-cli-integration: # Run CLI integration tests via SwiftPM
	swift test --filter MaestroCLIIntegrationTests

format: # Format all Swift code with swift-format (full-tree cleanup)
	swift-format -p --in-place --recursive --configuration ./.swift-format.json Maestro MaestroTests
...
format-changed: # Format Swift files changed from FORMAT_BASE_REF (default: origin/main)
	...
	files="$$( \
		{ \
			git diff --name-only --diff-filter=ACMR "$$base" -- Maestro MaestroTests; \
			git ls-files --others --exclude-standard -- Maestro MaestroTests; \
		} | awk '/\.swift$$/' | sort -u \
	)"; \
	if [ -z "$$files" ]; then \
		...
		printf '%s\n' "$$files" | xargs swift-format -p --in-place --configuration ./.swift-format.json; \
	fi

format-lint: # Check Swift formatting without rewriting files
	swift-format lint --strict --recursive --configuration ./.swift-format.json Maestro MaestroTests
...
bump-version: # Bump app version (usage: make bump-version [VERSION=YYYY.M.DD] [BUILD=YYYYMMDD])
	...
	if [ -z "$(BUILD)" ]; then \
		...
		current_build="$$(/usr/bin/awk -F' = ' '/CURRENT_PROJECT_VERSION = [0-9]+;/{gsub(/;/,"",$$2);print $$2; exit}' "$(CURRENT_MAKEFILE_DIR)/Maestro.xcodeproj/project.pbxproj")"; \
	...
	sed -i '' "s/CURRENT_PROJECT_VERSION = [0-9]*;/CURRENT_PROJECT_VERSION = $$build;/g" \
		"$(CURRENT_MAKEFILE_DIR)/Maestro.xcodeproj/project.pbxproj"; \
	printf '// Auto-generated by Makefile (sync-cli-version). Do not edit.\n\npublic enum MaestroVersion {\n  public static let current = "%s"\n}\n' "$$version" > \
		"$(CURRENT_MAKEFILE_DIR)/Maestro/CLIService/Shared/MaestroVersion.swift"; \
	git add "$(CURRENT_MAKEFILE_DIR)/Maestro.xcodeproj/project.pbxproj" \
		"$(CURRENT_MAKEFILE_DIR)/Maestro/CLIService/Shared/MaestroVersion.swift"; \
