...
let cliDefaultInstallPath = URL(fileURLWithPath: "/usr/local/bin/maestro")
...
extension CLIInstallClient: DependencyKey {
  static let liveValue = CLIInstallClient(
    bundledCLIURL: {
      Bundle.main.resourceURL?.appendingPathComponent("maestro-cli/maestro")
    },
    installationStatus: { installPath in
      ...
      let bundledURL = Bundle.main.resourceURL?.appendingPathComponent("maestro-cli/maestro")
    ...
    install: { installPath in
      let fileManager = FileManager.default
      guard let bundledURL = Bundle.main.resourceURL?.appendingPathComponent("maestro-cli/maestro") else {
