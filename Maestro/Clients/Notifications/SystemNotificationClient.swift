...
private nonisolated let notificationWorktreeIDKey = "maestro.worktreeID"
private nonisolated let notificationSurfaceIDKey = "maestro.surfaceID"
...
struct SystemNotificationClient {
  ...
  /// app icon" switch — it does not depend on Maestro's own banner toggle.
  enum DockBadgeAuthorization: Equatable {
    /// Notifications are allowed and "Badge app icon" is on.
    case available
    /// macOS is not allowing notifications for Maestro (denied or not yet asked).
