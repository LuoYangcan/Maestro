...
@MainActor
private final class MaestroAppStoreBox {
...
private enum GhosttyCLI {
  static func argv(resolvedKeybindings: ResolvedKeybindingMap) -> [UnsafeMutablePointer<CChar>?] {
    ...
    let executable = CommandLine.arguments.first ?? "maestro"
...
@MainActor
final class MaestroAppDelegate: NSObject, NSApplicationDelegate {
...
struct MaestroApp: App {
  @NSApplicationDelegateAdaptor(MaestroAppDelegate.self) private var appDelegate
  ...
  private static func cliLaunchOpenPath() -> String? {
    ...
    guard let flagIndex = args.firstIndex(of: MaestroSocket.cliOpenPathArgument),
  ...
  @MainActor init() {
    ...
    let storeBox = MaestroAppStoreBox()
  ...
  private static func makeCLISocketServer(
    ...
    do {
      try cliServer.start()
      logger.info("CLI socket server started at \(MaestroSocket.defaultPath)")
    } catch {
  ...
  var body: some Scene {
    Window("Maestro", id: WindowID.main) {
    ...
    .commands {
      ...
      CommandGroup(after: .appSettings) {
        ...
        .help("Install the maestro command line tool to /usr/local/bin")
      }
      #if DEBUG
        CommandMenu("Debug") {
          Button("Icon Catalog") {
      ...
      CommandGroup(replacing: .appTermination) {
        ...
        .help(helpText(title: "Quit Maestro", commandID: AppShortcuts.CommandID.quitApplication))
      }
    }
  }
