...
enum WindowLifecycleDiagnostics {
  ...
  private static let windowlessReportThreshold: TimeInterval = 10.0
  private static let windowlessStallReportThreshold: TimeInterval = 5.0
  ...
  private static func scheduleWindowlessReminder() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + windowlessReminderInterval) {
      MainActor.assumeIsolated {
        ...
        if seconds >= windowlessReportThreshold {
      ...
      MainActor.assumeIsolated {
        ...
        if lag >= stallThreshold {
          ...
          if windowlessSince != nil {
            ...
            if lag >= windowlessStallReportThreshold {
  ...
  static func applyLaunchStallIfConfigured() {
    let seconds = UserDefaults.standard.double(forKey: "MaestroDebugLaunchStallSeconds")
    guard seconds > 0 else { return }
    log("DEBUG launch stall: blocking main thread for \(seconds)s (MaestroDebugLaunchStallSeconds)")
  ...
  private static func reportWindowlessTimeoutIfNeeded(elapsed: TimeInterval) {
    ...
    reportWindowLifecycleEvent(kind: "main_window_timeout", elapsed: elapsed, lag: nil)
  }
  ...
  private static func reportWindowlessStallIfNeeded(lag: TimeInterval) {
    ...
    reportWindowLifecycleEvent(kind: "windowless_main_thread_stall", elapsed: elapsed, lag: lag)
  }

  private static func reportWindowLifecycleEvent(kind: String, elapsed: TimeInterval, lag: TimeInterval?) {
    let context = windowlessContext ?? "unknown"
    var fields = [
      String(format: "window lifecycle report kind=%@ elapsed=%.3fs context=%@", kind, elapsed, context),
      "active=\(NSApp.isActive)",
      "opener_registered=\(MainWindowOpener.shared.hasRegisteredOpener)",
      "max_lag=\(maxHeartbeatLagDuringWindowless)",
      "windows=\(windowsSummary())",
    ]
    if let lag {
      fields.append("lag=\(lag)")
    }
    logger.warning(fields.joined(separator: " "))
    #if DEBUG
      log(
