...
extension UTType {
  nonisolated static let maestroSidebarDragPayload = UTType.plainText
}

enum SidebarDragProvider {
  private nonisolated static let repositoryPrefix = "maestro-sidebar-repository:"
  private nonisolated static let worktreePrefix = "maestro-sidebar-worktree:"
  ...
  private nonisolated static func itemProvider(payload: String) -> NSItemProvider {
    ...
    provider.registerDataRepresentation(
      forTypeIdentifier: UTType.maestroSidebarDragPayload.identifier,
      visibility: .all,
...
struct SidebarRepositoryDropDelegate: DropDelegate {
  ...
  func performDrop(info: DropInfo) -> Bool {
    ...
    guard let provider = info.itemProviders(for: [.maestroSidebarDragPayload]).first else {
      actions.onDragEnded()
      return false
    }
    provider.loadDataRepresentation(forTypeIdentifier: UTType.maestroSidebarDragPayload.identifier) { data, _ in
      guard let data,
