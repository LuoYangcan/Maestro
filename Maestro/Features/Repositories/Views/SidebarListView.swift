...
struct SidebarListView: View {
  ...
  var body: some View {
    ...
    let agentWorktreeMetadata = ActiveAgentRowDisplayResolver.worktreeMetadata(
      repositories: state.repositories,
      customTitles: state.repositoryCustomTitles,
      repositoryAppearances: repositoryAppearances
    )
    let agentRowDisplays = ActiveAgentRowDisplayResolver.displays(
...
// MARK: - Previews
  ...
  private struct SidebarLayoutPreview: View {
    ...
    private static var mockState: RepositoriesFeature.State {
      let repo1Root = URL(fileURLWithPath: "/tmp/maestro")
      ...
      let repo1 = Repository(
        id: repo1Root.path, rootURL: repo1Root, name: "maestro", worktrees: repo1Worktrees
      )
