        ...
        return .merge(
          mergedWorktreeIDs.map { worktreeID in
            let shouldDeleteBranch =
              ...
              && state.maestroCreatedWorktreeIDs.contains(worktreeID)
            return .send(
              .worktreeLifecycle(
      ...
      guard let worktree = state.worktree(for: worktreeID),
        ...
        return .send(
          .presentAlert(
            ...
            message: "Maestro could not find the selected repository."
          )
        )
      }
          ...
          guard let repositoryURL = await gitClient.repositoryWebURL(repoRoot) else {
            await send(
              .presentAlert(
                ...
                message: "Maestro could not determine a code host URL for this repository."
              )
            )
            return
          }
      ...
      guard let pullRequest = optionalPullRequest else {
        return .send(
          .presentAlert(
            ...
            message: "Maestro could not find a pull request for this worktree."
          )
        )
      }
        ...
        guard let failingCheckDetailsURL, !failingCheckDetailsURL.isEmpty else {
          return .send(
            .presentAlert(
              ...
              message: "Maestro could not find a failing check URL."
            )
          )
        }
        return .run { send in
      ...
      case .openFailingCheckDetails:
        guard let failingCheckDetailsURL, let url = URL(string: failingCheckDetailsURL) else {
          return .send(
            .presentAlert(
              title: "Failing check not found",
              message: "Maestro could not find a failing check with details."
            )
          )
        }
          ...
          guard !branchName.isEmpty else {
            await send(
              .presentAlert(
                ...
                message: "Maestro could not determine the pull request branch."
              )
            )
            return
          }
            ...
            guard let run = try await githubCLI.latestRun(worktreeRoot, branchName) else {
              ...
              await send(
                .presentAlert(
                  ...
                  message: "Maestro could not find any workflow runs for this branch."
                )
              )
              return
            }
            guard run.conclusion?.lowercased() == "failure" else {
              ...
              await send(
                .presentAlert(
                  ...
                  message: "Maestro could not find a failing workflow run to copy logs from."
                )
              )
              return
            }
          ...
          do {
            ...
            guard run.conclusion?.lowercased() == "failure" else {
              ...
              await send(
                .presentAlert(
                  title: "No failing workflow run",
                  message: "Maestro could not find a failing workflow run to re-run."
                )
              )
              return
            }
