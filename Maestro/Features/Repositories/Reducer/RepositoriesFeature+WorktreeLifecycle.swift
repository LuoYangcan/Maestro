...
private func makeDeleteWorktreeConfirmation(
  ...
  let allMaestroCreated = targets.allSatisfy { target in
    state.maestroCreatedWorktreeIDs.contains(target.worktreeID)
  }
  let defaultDeleteBranch = settingsFile.global.deleteBranchOnDeleteWorktree && allMaestroCreated
  if count == 1,
