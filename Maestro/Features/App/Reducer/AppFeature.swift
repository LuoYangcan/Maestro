    ...
    let core = Reduce<State, Action> { state, action in
      ...
      case .appLaunched:
        do {
          ...
          appLogger.warning("Unable to migrate legacy cache files: \(error.localizedDescription)")
        }
      ...
      case .settings(.delegate(.cliInstallCompleted(let result))):
        ...
        case .installed(let path):
          return .send(.repositories(.showToast(.success("maestro installed at \(path)"))))
        case .uninstalled:
          return .send(.repositories(.showToast(.success("maestro command line tool removed"))))
