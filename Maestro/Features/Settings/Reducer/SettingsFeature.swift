  ...
  struct State: Equatable {
    ...
    init(settings: GlobalSettings = .default) {
      ...
      defaultWorktreeBaseDirectoryPath =
        MaestroPaths.normalizedWorktreeBaseDirectoryPath(settings.defaultWorktreeBaseDirectoryPath) ?? ""
    ...
    var globalSettings: GlobalSettings {
      GlobalSettings(
        ...
        defaultWorktreeBaseDirectoryPath: MaestroPaths.normalizedWorktreeBaseDirectoryPath(
          defaultWorktreeBaseDirectoryPath
        ),
  ...
  var body: some Reducer<State, Action> {
    ...
    Reduce { state, action in
        ...
        if state.cliInstallShowAlert {
          if path.isEmpty {
            state.alert = AlertState {
              ...
              TextState("The maestro command line tool has been removed.")
            }
          } else {
            state.alert = AlertState {
              ...
              TextState("The maestro command is now available at \(path).")
            }
          }
        }
      ...
      case .showNotificationPermissionAlert:
        state.alert = AlertState {
          TextState("Maestro cannot send system notifications")
        } actions: {
          ...
          TextState(
            "Notification permission is turned off. Open System Settings to allow Maestro to send notifications."
          )
        }
        return .none
