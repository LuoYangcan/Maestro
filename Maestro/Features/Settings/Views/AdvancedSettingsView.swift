      ...
      Form {
        Section("Command Line Tool") {
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Install the maestro command to control Maestro from the terminal.")
              .foregroundStyle(.secondary)
              .font(.callout)
            ...
            HStack(spacing: 8) {
              ...
              case .notInstalled:
                ...
                .help("Install maestro command line tool to /usr/local/bin")
                .buttonStyle(.bordered)
              case .installed:
                Button("Uninstall") {
                  store.send(.uninstallCLIButtonTapped)
                }
                .help("Remove maestro command line tool from /usr/local/bin")
                .buttonStyle(.bordered)
              case .installedDifferentSource:
                Button("Reinstall") {
                  store.send(.installCLIButtonTapped())
                }
                .help("Replace the existing maestro command with the version bundled in this app")
                .buttonStyle(.bordered)
              }
            }
          }
        ...
        Section("Advanced") {
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("When enabled, Maestro attempts to restore tabs and splits after restart.")
              .foregroundStyle(.secondary)
              .font(.callout)
