...
nonisolated struct RepositorySettingsKey: SharedKey {
  ...
  func load(
    ...
    for legacySettingsURL in MaestroPaths.legacyRepositorySettingsURLs(for: rootURL) {
      guard let legacyData = try? repositoryLocalSettingsStorage.load(legacySettingsURL) else {
        continue
      }
