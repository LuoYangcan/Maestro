...
private func globalCommandItems(showsNewWorktreeAction: Bool) -> [CommandPaletteItem] {
  ...
  items.append(
    .appShortcut(
      ...
      category: .app,
      kind: .installCLI,
      keywords: ["cli", "command line", "terminal", "maestro"]
    )
  )
...
/// Ghostty action keys that should be hidden from Maestro's command palette.
///
/// Includes:
/// - actions Maestro already exposes natively (`check_for_updates`)
/// - Ghostty actions intentionally unsupported by Maestro's architecture/platform
