...
final class GhosttyRuntime {
  ...
  // Maestro constructs a single GhosttyRuntime for the whole app lifetime
  // (see `MaestroApp.init`). This weak reference gives UI surfaces that
  ...
  /// Re-reads the user's Ghostty config from disk and re-applies Maestro's
  ...
  private func applyRuntimeOverridesIfNeeded() {
    ...
    if !appKeybindOverrideContents.isEmpty {
      let url = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("maestro-ghostty-keybind-overrides.conf")
      do {
    ...
    if !themeFallbackOverrideContents.isEmpty {
      let url = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("maestro-ghostty-theme-overrides.conf")
      do {
  ...
  /// Reads a Maestro-specific override for the visible divider thickness from the
  ...
  /// so we layer a `maestro-split-divider-width = N` directive on top of the
  /// user's existing Ghostty config to avoid carrying another fork patch.
  func splitDividerWidth() -> CGFloat? {
    Self.parseMaestroSplitDividerWidth(at: Self.ghosttyConfigPath())
  }
  ...
  nonisolated static func parseMaestroSplitDividerWidth(at path: String?) -> CGFloat? {
    guard let path else { return nil }
    guard let contents = try? String(contentsOfFile: path, encoding: .utf8) else { return nil }
    return parseMaestroSplitDividerWidth(from: contents)
  }

  nonisolated static func parseMaestroSplitDividerWidth(from contents: String) -> CGFloat? {
    let key = "maestro-split-divider-width"
