// MaestroCLI/Commands/OpenCommand.swift
...
struct OpenCommand: ParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "open",
    abstract: "Open a path in Maestro, or bring the app to front."
  )

  @OptionGroup var options: GlobalOptions

  @Argument(help: "Path to open. Omit to bring Maestro to front.")
  var path: String?
  ...
  mutating func run() throws {
    try CLIExecution.run(command: "open", output: options.outputMode) {
      ...
      if let resolvedPath {
        setenv(MaestroSocket.cliOpenPathEnvironmentKey, resolvedPath, 1)
      } else {
        unsetenv(MaestroSocket.cliOpenPathEnvironmentKey)
      }
  ...
  /// Check if Maestro is reachable via socket; if not, launch it.
