// MaestroCLI/AppLauncher.swift
// Launches Maestro.app when CLI detects the app is not running, then waits
...
enum AppLauncher {
  ...
  static func ensureAppRunning() throws -> Bool {
    ...
    if let override = ProcessInfo.processInfo.environment[MaestroSocket.environmentKey],
      !override.isEmpty
    {
      return false
    }
  ...
  /// Check whether a Maestro process is currently running via `pgrep`.
  private static func isAppProcessRunning() -> Bool {
    let process = Process()
    process.executableURL = URL(fileURLWithPath: "/usr/bin/pgrep")
    process.arguments = ["-x", "Maestro"]
  ...
  private static func launchApp() throws {
    ...
    do {
      ...
      throw ExitError(
        ...
        message: "Failed to launch Maestro: \(error.localizedDescription)"
      )
    }
    guard process.terminationStatus == 0 else {
      throw ExitError(
        code: CLIErrorCode.launchFailed,
        message: "Failed to launch Maestro (exit code \(process.terminationStatus))."
      )
    }
  }
  ...
  private static func launchArguments() -> [String] {
    var args = ["-a", "Maestro"]
    if let openPath = ProcessInfo.processInfo.environment[MaestroSocket.cliOpenPathEnvironmentKey],
      !openPath.isEmpty
    {
      args.append(contentsOf: ["--args", MaestroSocket.cliOpenPathArgument, openPath])
    }
    return args
  }
  ...
  private static func waitForSocket() throws {
    let socketPath = MaestroSocket.defaultPath
    ...
    throw ExitError(
      code: CLIErrorCode.launchFailed,
      message: "Maestro CLI socket did not become available within \(Int(socketTimeoutSeconds))s."
    )
  }
