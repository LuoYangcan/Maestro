#!/usr/bin/env bash
# Maestro release script: bump, build, sign, notarize, and publish.
#
...
#   SPARKLE_PRIVATE_KEY_FILE        Path to EdDSA private key file (default: ~/.maestro-sparkle-private-key)
#   NETLIFY_BUILD_HOOK              Netlify Build Hook URL for Maestro-Site rebuild
set -euo pipefail
...
SPARKLE_KEY_FILE="${SPARKLE_PRIVATE_KEY_FILE:-$HOME/.maestro-sparkle-private-key}"
...
# render as <h3> on Maestro-Site (its CSS targets `:global(h3)`) and sit one
...
BUILD="$(date +%Y%m%d)"
CURRENT_BUILD="$(/usr/bin/awk -F' = ' '/CURRENT_PROJECT_VERSION = [0-9]+;/{gsub(/;/,""); print $2; exit}' "$PROJECT_DIR/Maestro.xcodeproj/project.pbxproj")"
...
MAESTRO_CLI="$APP_PATH/Contents/Resources/maestro-cli/maestro"
if [[ -f "$MAESTRO_CLI" ]]; then
  log "re-signing bundled maestro CLI with hardened runtime..."
  codesign -f -s "$IDENTITY_SHA" -o runtime --timestamp -v "$MAESTRO_CLI"
fi
...
ZIP_PATH="build/Maestro.app.zip"
...
# ── Trigger Maestro-Site rebuild ───────────────────────────────────────────────

log "triggering Maestro-Site rebuild..."
if [[ -n "${NETLIFY_BUILD_HOOK:-}" ]]; then
  curl -fsSL -X POST "$NETLIFY_BUILD_HOOK" 2>/dev/null \
    && log "Maestro-Site rebuild triggered" \
    || log "Maestro-Site rebuild trigger failed (non-critical)"
else
  log "NETLIFY_BUILD_HOOK not set, skipping Maestro-Site rebuild"
fi
