...
struct MaestroAppCLITests {
  @Test func cliRouterWiresKeyAndReadHandlersInsteadOfStubHandlers() async {
    ...
    let router = MaestroApp.makeCLICommandRouter(appStore: store, terminalManager: terminalManager)
  ...
  @Test func resolveCLITerminalWorktreeBuildsSyntheticRunnableFolderWorktree() {
    ...
    let resolved = MaestroApp.resolveCLITerminalWorktree(
      id: repository.id,
