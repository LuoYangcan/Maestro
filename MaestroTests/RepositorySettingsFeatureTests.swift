...
struct RepositorySettingsFeatureTests {
  @Test(.dependencies) func plainFolderTaskLoadsWithoutGitRequests() async throws {
    ...
    let settingsFileURL = URL(fileURLWithPath: "/tmp/maestro-settings-\(UUID().uuidString).json")
    let expectedDefaultWorktreeBaseDirectoryPath =
      MaestroPaths.normalizedWorktreeBaseDirectoryPath("/tmp/worktrees")
    ...
    try #require(
      try? localStorage.save(
        userSettingsData,
        at: MaestroPaths.userRepositorySettingsURL(for: rootURL)
      )
    )
  ...
  @Test(.dependencies) func conflictingCustomShortcutPersistsAsUserOverride() async throws {
    ...
    let savedData = try #require(localStorage.data(at: MaestroPaths.userRepositorySettingsURL(for: rootURL)))
  ...
  @Test(.dependencies) func customTitleBindingPersistsToRepositoryFile() async throws {
    ...
    let localStorage = RepositoryLocalSettingsTestStorage()
    let settingsFileURL = URL(fileURLWithPath: "/tmp/maestro-settings-\(UUID().uuidString).json")
    let repositorySettingsURL = MaestroPaths.repositorySettingsURL(for: rootURL)
