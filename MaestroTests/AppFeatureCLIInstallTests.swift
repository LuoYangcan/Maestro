...
struct SettingsFeatureCLIInstallTests {
  @Test(.dependencies) func installShowsSuccessAlert() async {
    ...
    let store = TestStore(
      ...
      $0.cliInstallClient.installationStatus = { _ in .installed(path: "/usr/local/bin/maestro") }
    }
    ...
    await store.receive(\.cliInstallCompleted.success) {
      $0.alert = AlertState {
        ...
        TextState("The maestro command is now available at /usr/local/bin/maestro.")
      }
      $0.cliInstallStatus = .installed(path: "/usr/local/bin/maestro")
    }
  ...
  @Test(.dependencies) func commandPaletteInstallRoutesToSettingsAndShowsToast() async {
    ...
    await store.receive(\.settings.cliInstallCompleted.success) {
      $0.settings.cliInstallStatus = .installed(path: "/usr/local/bin/maestro")
    }
    await store.receive(\.settings.delegate.cliInstallCompleted)
    await store.receive(\.repositories.showToast) {
      $0.repositories.statusToast = .success("maestro installed at /usr/local/bin/maestro")
    }
