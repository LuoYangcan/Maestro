...
struct RepositorySettingsKeyTests {
  ...
  @Test(.dependencies) func loadNormalizesLegacyDefaultOverridesToInheritedValues() throws {
    ...
    let localURL = MaestroPaths.repositorySettingsURL(for: rootURL)
    let legacyData = Data(
      """
      {
        "setupScript": "",
  ...
  @Test(.dependencies) func loadPrefersLocalMaestroJSONOverGlobalEntry() throws {
    let globalStorage = SettingsTestStorage()
    let localStorage = RepositoryLocalSettingsTestStorage()
    let rootURL = URL(fileURLWithPath: "/tmp/repo")
    let settingsFileURL = URL(fileURLWithPath: "/tmp/maestro-settings-\(UUID().uuidString).json")
    ...
    try localStorage.save(
      ...
      at: MaestroPaths.repositorySettingsURL(for: rootURL)
    )
  ...
  @Test(.dependencies) func loadMigratesLegacyRepositoryRootFile() throws {
    ...
    let settingsFileURL = URL(fileURLWithPath: "/tmp/maestro-settings-\(UUID().uuidString).json")
    let localURL = MaestroPaths.repositorySettingsURL(for: rootURL)
    let legacyURL = MaestroPaths.legacyRepositorySettingsURL(for: rootURL)
