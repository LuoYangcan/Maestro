...
struct CLIKeyCommandHandlerTests {
  ...
  @Test func successfulKeyDelivery() async throws {
    ...
    #expect(response.schemaVersion == "maestro.cli.key.v1")
  ...
  @Test func targetNotUniqueError() async throws {
    let handler = Self.makeHandler(
      resolveResult: .failure(.notUnique("Worktree 'Maestro' matches 2 worktrees."))
    )
    let response = await handler.handle(
      envelope: Self.makeEnvelope(selector: .worktree("Maestro"))
    )
