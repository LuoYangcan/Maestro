...
struct RepositoryIconAssetStoreTests {
  ...
  private func makeRepoRootScratch() -> ScratchDirectory {
    ScratchDirectory(prefix: "maestro-icon-store")
  }

  private func makeStoreFixture() -> StoreFixture {
    let repoRoot = makeRepoRootScratch()
    let iconStoreRoot = ScratchDirectory(prefix: "maestro-icon-store-home")
  ...
  @Test func importImageCopiesFileWithUUIDName() throws {
    ...
    let source = ScratchDirectory(prefix: "maestro-icon-source")
    ...
    let productionURL = MaestroPaths.repositoryIconFileURL(
