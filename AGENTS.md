...
`ActiveAgentsFeature` (`@Reducer`, nested in `RepositoriesFeature`) holds an `IdentifiedArrayOf<ActiveAgentEntry>` plus a focus anchor; the roster is kept sorted by displayed repository → worktree → branch → per-agent creation order, re-sorted in the parent `RepositoriesFeature` (`resortActiveAgents`) since only it can resolve the display names. Data flows OUT of the terminal layer: `WorktreeTerminalState` polls each surface, builds a `PaneAgentState` whose derived `displayState` is an `AgentDisplayState` (working / blocked / done / idle), and emits `TerminalClient.Event.agentEntryChanged` / `.agentEntryRemoved`. `AppFeature.task` drains the event stream and relays into `.repositories(.activeAgents(...))`. Clicking a row sends `.activeAgents(.entryTapped(id))`; `RepositoriesFeature` intercepts it to call `terminalClient.focusSurface` then select the worktree/repo. The roster renders both as a resizable sidebar panel (`ActiveAgentsPanel`) and a system menu-bar item (`ActiveAgentsStatusItemController`, created in `MaestroAppDelegate.applicationDidFinishLaunching`, drawing stacked red/green status dots). Each row's displayed repository/worktree/branch is resolved by `ActiveAgentRowDisplayResolver.displayWorktreeID(for:in:)`: it first geometrically matches the agent's live process cwd to the deepest registered worktree (`resolveWorktreeID`), then clamps to the tab's **owning** worktree whenever that match lands in the **same** repository (ancestor, descendant, sibling, or the worktree itself all show the owning worktree); only a cwd inside a **different** repository overrides the label to show that repo, and a cwd outside every registered repository also falls back to the owning worktree. `ActiveAgentEntrySorter` routes its sort key through the same `displayWorktreeID(for:in:)`, so a row's sort group can never diverge from its rendered label. The sidebar Refresh button (`refreshWorktrees`) also sends `TerminalClient.Command.resyncActiveAgents`, forcing the terminal layer to re-read every detected agent's cwd and re-emit (no-op while detection is disabled). Detection is gated via `TerminalClient.Command.setAgentDetectionEnabled` — skipped when the panel is hidden and `autoShowActiveAgentsPanel` is off. Keybindings: ⌘⌥P toggles the panel; ⌃⌥↑/↓ step through agents.
...
- **Commands**: `createTab`, `closeFocusedTab`, `prune`, `setSelectedWorktreeID`, `setAgentDetectionEnabled`, `resyncActiveAgents`, etc.
