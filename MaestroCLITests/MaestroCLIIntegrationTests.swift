...
final class MaestroCLIIntegrationTests: XCTestCase {
  ...
  func testHelpAndVersionSmoke() throws {
    let version = try runMaestro(args: ["--version"])
    XCTAssertEqual(version.exitCode, 0)
    XCTAssertTrue(version.stdout.contains(MaestroVersion.current))

    let help = try runMaestro(args: ["--help"])
  ...
  func testListReturnsAppNotRunningWhenSocketUnavailable() throws {
    ...
    let result = try runMaestro(
      args: ["list", "--json"],
      environment: [MaestroSocket.environmentKey: socketPath]
    )
  ...
  func testOpenCommandRoundTripsOverSocket() throws {
    ...
    let response = try CommandResponse(
      ok: true,
      command: "open",
      schemaVersion: "maestro.cli.open.v1",
      data: RawJSON(
  ...
  func testFocusCommandRoundTripsOverSocket() throws {
    ...
    let response = CommandResponse(
      ok: true,
      command: "focus",
      schemaVersion: "maestro.cli.focus.v1"
    )
  ...
  func testFocusRejectsMultipleSelectorsBeforeTransport() throws {
    let result = try runMaestro(args: ["focus", "--worktree", "Maestro", "--pane", "pane-123", "--json"])
  ...
  func testFocusCommandTextRenderingFromSocket() throws {
        ...
        encoding: FocusResponseData(
          ...
          target: FocusResponseTarget(
            worktree: ListWorktree(
              ...
              rootPath: "/Users/onevcat/Projects/Maestro",
              kind: "git"
            ),
    ...
    XCTAssertEqual(result.exitCode, 0)
    XCTAssertTrue(result.stdout.contains("Focused Maestro:Maestro"), "Missing focus header: \(result.stdout)")
    ...
    XCTAssertTrue(result.stdout.contains("tab: Maestro 1"), "Missing tab field: \(result.stdout)")
  }
  ...
  func testListCommandTextRenderingFromSocket() throws {
    ...
    let response = try CommandResponse(
      ok: true,
      command: "list",
      schemaVersion: "maestro.cli.list.v1",
      data: RawJSON(
    ...
    XCTAssertEqual(result.exitCode, 0)
    XCTAssertTrue(result.stdout.contains("Maestro:Maestro (running)"), "Missing worktree header: \(result.stdout)")
  ...
  func testSendCommandRoundTripsOverSocket() throws {
    ...
    let response = try CommandResponse(
      ok: true,
      command: "send",
      schemaVersion: "maestro.cli.send.v1",
      data: RawJSON(
        encoding: SendResponseData(
          target: SendResponseTarget(
            worktree: ListWorktree(
              id: "Maestro:/Projects/Maestro", name: "Maestro",
              path: "/Projects/Maestro", rootPath: "/Projects/Maestro", kind: "git"
            ),
            tab: SendResponseTab(id: "2FC00CF0-3974-4E1B-BEF8-7A08A8E3B7C0", title: "Maestro 1", selected: true),
            pane: SendResponsePane(
              id: "6E1A2A10-D99F-4E3F-920C-D93AA3C05764",
              title: "zsh", cwd: "/Projects/Maestro", focused: true
            )
          ),
  ...
  func testSendEmptyInputReturnsError() throws {
    let result = try runMaestro(args: ["send", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    let error = try XCTUnwrap(payload["error"] as? [String: Any])
  ...
  func testSendTimeoutValidation() throws {
    let result = try runMaestro(args: ["send", "hello", "--timeout", "0", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    let error = try XCTUnwrap(payload["error"] as? [String: Any])
  ...
  func testKeyCommandRoundTripsOverSocket() throws {
    ...
    let response = try CommandResponse(
      ok: true,
      command: "key",
      schemaVersion: "maestro.cli.key.v1",
      data: RawJSON(
        encoding: KeyResponseData(
          ...
          target: KeyResponseTarget(
            ...
            tab: KeyResponseTab(id: "2FC00CF0-3974-4E1B-BEF8-7A08A8E3B7C0", title: "Maestro 1", selected: true),
    ...
    XCTAssertEqual(payload["schema_version"] as? String, "maestro.cli.key.v1")
    let data = try XCTUnwrap(payload["data"] as? [String: Any])
  ...
  func testKeyCommandRejectInvalidRepeatZero() throws {
    let result = try runMaestro(args: ["key", "enter", "--repeat", "0", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    XCTAssertEqual(payload["command"] as? String, "key")
  ...
  func testKeyCommandRejectInvalidRepeatOver100() throws {
    let result = try runMaestro(args: ["key", "enter", "--repeat", "101", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    let error = try XCTUnwrap(payload["error"] as? [String: Any])
  ...
  func testKeyCommandRejectUnsupportedKey() throws {
    let result = try runMaestro(args: ["key", "hyper-k", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    XCTAssertEqual(payload["command"] as? String, "key")
  ...
  func testKeyCommandRejectMultipleSelectors() throws {
    let result = try runMaestro(args: ["key", "enter", "--worktree", "Maestro", "--pane", "pane-123", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    XCTAssertEqual(payload["command"] as? String, "key")
  ...
  func testKeyCommandTextRenderingFromSocket() throws {
      ...
      data: RawJSON(
        encoding: KeyResponseData(
          ...
          target: KeyResponseTarget(
            ...
            tab: KeyResponseTab(id: "t1", title: "Maestro 1", selected: true),
            pane: KeyResponsePane(id: "p1", title: "Claude", cwd: "/Projects/Maestro", focused: true)
          )
        ))
    )
    ...
    XCTAssertTrue(result.stdout.contains("Maestro:Maestro"), "Missing worktree: \(result.stdout)")
  ...
  func testKeyCommandRejectsUnsupportedShiftedSymbolLiteral() throws {
    let result = try runMaestro(args: ["key", "!", "--json"])
    XCTAssertNotEqual(result.exitCode, 0)
    let payload = try jsonObject(from: result.stdout)
    XCTAssertEqual(payload["ok"] as? Bool, false)
    XCTAssertEqual(payload["command"] as? String, "key")
  ...
  func testReadCommandRoundTripsOverSocket() throws {
    ...
    let response = try CommandResponse(
      ok: true,
      command: "read",
      schemaVersion: "maestro.cli.read.v1",
      data: RawJSON(
        encoding: ReadResponseData(
          target: ReadResponseTarget(
            worktree: ListWorktree(
              id: "Maestro:/Projects/Maestro",
              name: "Maestro",
              path: "/Projects/Maestro",
              rootPath: "/Projects/Maestro",
              kind: "git"
            ),
            tab: ReadResponseTab(
              id: "2FC00CF0-3974-4E1B-BEF8-7A08A8E3B7C0",
    ...
    XCTAssertEqual(payload["schema_version"] as? String, "maestro.cli.read.v1")
  }
  ...
  func testReadRejectsInvalidLastBeforeTransport() throws {
    let result = try runMaestro(args: ["read", "--last", "0", "--json"])
  ...
  func testReadRejectsMultipleSelectorsBeforeTransport() throws {
    let result = try runMaestro(args: ["read", "--worktree", "Maestro", "--pane", "pane-123", "--json"])
  ...
  private func runMaestro(
    args: [String],
    environment: [String: String] = [:],
    stdinData: Data? = nil
  ) throws -> CommandResult {
    let binaryPath = try ensureMaestroBinary()
  ...
  private func ensureMaestroBinary() throws -> String {
    let candidates = [
      repoRoot.appendingPathComponent(".build/debug/maestro").path,
      repoRoot.appendingPathComponent(".build/arm64-apple-macosx/debug/maestro").path,
      repoRoot.appendingPathComponent(".build/x86_64-apple-macosx/debug/maestro").path,
    ]
    ...
    throw NSError(
      domain: "MaestroCLITests",
      code: 1,
      userInfo: [
        NSLocalizedDescriptionKey: "Could not find maestro binary. Checked: \(candidates.joined(separator: ", "))"
      ]
    )
  }
  ...
  private func temporarySocketPath(suffix: String) -> String {
    ...
    let filename = "maestro-cli-\(suffix)-\(uuid).sock"
